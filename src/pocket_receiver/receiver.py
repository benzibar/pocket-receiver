"""Robust rtl_fm -> ALSA process pipeline with automatic mixer control."""
...
def pcm_level_dbfs(samples: array.array[int]) -> float:
    ...
    return max(
        ...
        min(
            0.0,
            20.0 * math.log10(math.sqrt(mean_square) / 32768.0),
        ),
    )
...
class ReceiverPipeline:
    ...
    def _rtl_command(self) -> list[str]:
        ...
        command = [
            ...
            str(self.device),
            "-f",
            str(round(s.frequency_mhz * 1_000_000)),
            "-M",
            s.mode.lower(),
            "-s",
            str(bandwidth_hz(s.bandwidth)),
            "-r",
            "48000",
        ]
    ...
    def check_dependencies(self) -> None:
        missing = [
            name
            for name in ("rtl_fm", "aplay", "amixer")
    ...
    def _detect_mixer_control(self) -> str:
        """Return a supported ALSA output volume control."""
        ...
        if result.returncode != 0:
            ...
            raise RuntimeError(
                f"Unable to query ALSA mixer controls: {detail}"
            )

        available = result.stdout

        for control in ("Master", "PCM"):
            if f"'{control}'" in available:
                return control

        raise RuntimeError(
            "No supported ALSA volume control found. "
            "Expected 'Master' or 'PCM'."
        )

    def _set_system_volume(self, volume: int) -> None:
        """Map the UI volume to the available ALSA output control."""
        control = self._detect_mixer_control()
        ...
        if result.returncode != 0:
            ...
            raise RuntimeError(
                f"Unable to set ALSA {control} volume: {detail}"
            )
        ...
        with self._lock:
            ...
            self._threads = [
                ...
                threading.Thread(
                    ...
                    args=(self._rtl, "RTL-SDR"),
                    daemon=True,
                ),
                threading.Thread(
                    target=self._read_errors,
                    args=(self._aplay, "Audio"),
                    daemon=True,
                ),
                threading.Thread(
            ...
            self.report(
                f"Playing "
                f"{self.settings.frequency_mhz:.3f} MHz "
        ...
        while not self._stopping.is_set():
            ...
            if (
                ...
                detail = (
                    self.last_error
                    or (
                        f"{failed_name} exited "
                        f"with status {failed_code}"
                    )
                )

                self.report(
                    f"Receiver stopped: {detail}"
                )
